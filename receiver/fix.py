...
DPI = 203
VERTICAL_GAP = int(0.7 * DPI)
HORIZONTAL_GAP = int(0.2 * DPI)
LEFT_MARGIN = int(0.05 * DPI)
RIGHT_MARGIN = int(0.05 * DPI)
TOP_MARGIN = 0
BOTTOM_MARGIN = 0

INDIVIDUAL_LABEL_WIDTH = int(3.5 * DPI)                             # 3.5cm per label width
INDIVIDUAL_LABEL_HEIGHT = int(2.2 * DPI)                            # 2.2cm per label height
QRCODE_SIZE = int(INDIVIDUAL_LABEL_WIDTH // 2)                      # QR Code size: Half of label width, vertically centered
BORDER_GAP = int(0.2 * DPI)      # Gap between border & content, between QRcode & text
QR_TEXT_GAP = int(0.2 * DPI)
MAX_TEXT_WIDTH = INDIVIDUAL_LABEL_WIDTH - QRCODE_SIZE - (BORDER_GAP * 2) - QR_TEXT_GAP
MAX_TEXT_HEIGHT = INDIVIDUAL_LABEL_HEIGHT - (BORDER_GAP * 2)
...
def get_font_for_text(font_size, font_path=None): # Check the size
    ...
    return font
def print_image(image_path, sheet_width, sheet_height):
    """Send the print job to the printer using the lp command."""
    try:
        format = subprocess.run(
            ["lpadmin", "-p", PRINTER_NAME, "-o", f"media=Custom.{sheet_width}x{sheet_height}"],
            check=True,
...
def print_labels():
    ...
    try:
        ...
        num_rows = math.ceil(len(device_list) / LABEL_PER_ROW)  # Two labels per row
        num_cols = LABEL_PER_ROW
        num_rows = math.ceil(len(device_list) / LABEL_PER_ROW)  # Total rows based on number of devices
        sheet_height = (num_rows * INDIVIDUAL_LABEL_HEIGHT) + ((num_rows - 1) * VERTICAL_GAP) + TOP_MARGIN + BOTTOM_MARGIN
        ...
        for index, device in enumerate(device_list):
            ...
            if (index + 1) % 2 == 0:  # End of row
                ...
                if not print_image(final_image_path, sheet_width, sheet_height):
